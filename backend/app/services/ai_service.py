...
def _beneficiary_lines(beneficiaries: List[Dict[str, Any]], limit: int = 40) -> str:
    ...
    for item in beneficiaries[:limit]:
        ...
        lines.append(
            ...
            f"балл приоритетности: {_value(item.get('priority'))}"
        )
...
EXPLAIN_SYSTEM = (
    ...
    "Отвечайте ТОЛЬКО на русском языке. Английский язык недопустим "
    "ни в одном предложении, даже в заголовках и перечислениях."
)
...
БЕНЕФИЦИАРНЫЙ СОБСТВЕННИК
  ...
  Балл приоритетности: {_value(beneficiary.get('priority'))} (чем меньше, тем надёжнее признак)
...
ВАЖНО: ответ должен быть полностью на русском языке. Ни одного предложения
на английском."""
...
CHAT_SYSTEM = (
    ...
    "какие сведения нужно запросить дополнительно. Отвечайте ТОЛЬКО на русском "
    "языке. Английский язык недопустим ни в одном предложении."
)
