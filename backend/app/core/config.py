...
class Settings(BaseSettings):
    ...
    #: Дочищать ли моделью строки ФИО, с которыми не справились правила.
    #: Разобранное запоминается в PostgreSQL, поэтому к модели
    #: обращение идёт один раз на каждую новую строку.
    AI_NAME_CLEANUP: bool = True
    #: Сколько новых строк уходит к модели за один заход
    AI_NAME_CLEANUP_BATCH: int = 25
    ...
    # ---------------- Безопасность ----------------
    #: Требовать вход по логину и паролю. Вход обычный: логин, пароль,
    #: срок сессии. Никаких одноразовых кодов в системе нет.
    ...
    #:
    #: Сейчас выключено по просьбе заказчика. Код авторизации никуда
    #: не делся: чтобы вернуть вход, достаточно поставить true в .env
    #: и перезапустить — учётная запись администратора создаётся всё равно.
    ...
    #: Пароль администратора при первом запуске. Пустое значение — пароль
    #: будет выдан случайным и записан в журнал: постоянного пароля
    #: в открытом репозитории быть не должно.
    ...
    #: Сводная таблица бенефициаров — объединение результатов тех же
    #: алгоритмов, которое ведёт организация. Реестр берёт её основой,
    #: а таблицы отдельных алгоритмов дополняют её. Пустое значение
    #: или таблица неподходящей формы — читаются только алгоритмы.
    #:
    #: Имён можно перечислить несколько через запятую: берётся первое
    #: подходящее. Указана только итоговая таблица: AFM_6_1_99 и
    #: AFM_6_1_99_OLD — промежуточные, они сводятся в _merged, и читать
    #: их вместо неё значило бы брать часть данных.
    MERGED_TABLE: str = "AFM_6_TEST.AFM_6_1_99_merged"
