...
async def delete_user(username: str, session: SessionDep, admin: AdminUser) -> dict:
    ...
    # Учётную запись администратора удалить нельзя вовсе — ни свою, ни чужую.
    # Случайное удаление коллеги с полным доступом обходится дорого, а
    # намеренное остаётся возможным: сначала сменить роль на «Пользователь»,
    # и только потом удалять. Это уже осознанное действие в два шага, и оба
    # записываются в журнал. Заблокировать администратора можно и без
    # удаления — переключателем «Действует».
    if user.role == UserRole.ADMINISTRATOR.value:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            "Администратора удалить нельзя. Смените роль на «Пользователь» "
            "либо заблокируйте учётную запись.",
        )
